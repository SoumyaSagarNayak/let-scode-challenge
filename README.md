...
> **"How is India performing compared to the rest of the world — and where should policy focus next?"**

**India Global Scorecard** is a production-quality development intelligence platform consolidating trusted multilateral rankings (World Bank, UNDP, WIPO, WEF, WHO, UNESCO, Yale EPI, Transparency International, UN DESA, IEP, ITU) into a single interactive scorekeeping system. Explore 10 core developmental pillars, benchmark India side-by-side against 15 key global economies, calculate peer standing (`X / 16 Peers`), track micro-trend velocity (`▲ +2`), visualize 2018–2025 historical trajectories, and generate AI executive policy summaries.

---

## 🌟 Key Features

1. **Clay Design System**: Modern cream canvas typography, saturated multi-color feature cards (pink, teal, lavender, peach, ochre), glassmorphism, and responsive micro-interactions.
2. **10 Core Developmental Pillars**: Economy, Society, Governance, Education, Healthcare, Technology, Environment, Safety, Equality, and Digital Government.
3. **Peer Group Benchmarking**: Dynamic peer standing badges (`X / 16 Peers`) evaluating India relative to 16 benchmark economies (G20 + key regional neighbors).
4. **Micro-Trend Velocity Indicators**: Directional change badges (`▲ +2`, `▼ -1.5`, `● Flat`) displaying momentum and rank delta directly on cards.
5. **Consolidated Quick Indicator Detail Modal**: Clicking any card opens a preview modal with mini 5-year trend line area charts, side-by-side peer snapshot table, verified direct publisher links, and AI strategic analysis.
6. **Digestible Executive Insights**: Structured prose summary box on the main dashboard categorizing standings into "Strongest Standing" (Top vectors) vs "Priority Reform Areas" (Headroom for growth).
7. **Real-Time Instant Client Search & Pill Filtering**: Instant zero-latency search and pillar pill filter tabs across all 40+ indicators with zero page reloads.
8. **Bilateral Country Comparison**: Side-by-side comparative dashboard featuring Recharts Radar & Bar charts calculating relative strengths and headroom for reform.
9. **Interactive World Map**: Powered by `react-simple-maps` with D3 color choropleth scales, hover tooltips, and click-to-compare drawer.
10. **AI Executive Insights**: Context-aware summary generator producing executive policy insights under 150 words (integrates OpenAI API with intelligent fallback engine).
11. **Historical Trajectory (2018 - 2025)**: Interactive line/area charts tracking year-on-year rank shifts and metric growth percentages.
12. **Instant Global Search (Cmd+K / Ctrl+K)**: Global modal search with cross-field fuzzy indexing.
13. **Local Bookmarks & Theme Persistence**: Saved metrics and Dark/Light mode persisted locally in browser LocalStorage.
14. **Unified Vercel Serverless Deployment**: Deploys 100% FREE on Vercel as a single repo with Vercel Serverless Functions for `/api/*` routes.
...
│   ├── src/
│   │   ├── components/        # Clay UI & Chart components (includes IndicatorDetailModal)
...
└── backend/                   # Express.js REST API & JSON Datasets
    ...
    ├── utils/                 # Data search, peer rank & calculation engine (dataHelper.js)
